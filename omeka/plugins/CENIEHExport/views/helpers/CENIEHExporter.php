 ...
 Class CENIEHExporter {
   ...
   private $_unqualified = array(
    ...
    public function exportCollectionZip($collectionID) {
        include_once(dirname(dirname(__FILE__)).'/../libraries/ZipStream/ZipStream.php');

        $collection = get_record_by_id("collection",$collectionID);
        $items = get_records('Item',array('collection'=>$collectionID), 0); // ítems de la colección
        ...
        foreach($items as $item) { 
            ob_start();
            $this->_generateCeniehItem($item->id);
            $zip->add_file("Item_".$item->id.".xml", ob_get_clean() );
        }
