 ...
 * 
 */
abstract class CollectionFiles_File_Ingest_AbstractIngest
{
    /**
     * @var Collection
     */
    ...
    /**
     * Set the collection to use as a target when ingesting files.
     * 
     ...
     */
    final public static function factory($adapterName, $collection, $options = array())
    {
        $className = 'CollectionFiles_File_Ingest_' . $adapterName;
        if (class_exists($className, true)) {
            ...
            throw new CollectionFiles_File_Ingest_Exception('Could not load ' . $className);
        }
    }
    ...
    protected function _getDestination($fromFilename)
    {
        ...
        if (!is_writable($dir)) {
            throw new CollectionFiles_File_Ingest_Exception('Cannot write to the following directory: "'
                              . $dir . '"!');
        }
        return $dir . '/' . $filename;
    }
    ...
    protected function _validateFile($filePath, $fileInfo)
    {
        ...
        if (!empty($validationErrors)) {
            throw new CollectionFiles_File_Ingest_InvalidException(join("\n\n", array_values($validationErrors)));
        }
        return true;
    }
}
