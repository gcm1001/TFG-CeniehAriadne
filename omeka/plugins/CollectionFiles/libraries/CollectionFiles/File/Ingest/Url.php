 ...
 * 
 */
class CollectionFiles_File_Ingest_Url extends CollectionFiles_File_Ingest_AbstractSourceIngest
{
    /**
    ...
    protected function _transfer($source, $destination, array $fileInfo)
    {
        try {
            ...
            throw new CollectionFiles_File_Ingest_Exception(
    ...
    protected function _validateSource($source, $info)
    {
        ...
        if (!($uri && $uriIsValid)) {
            throw new CollectionFiles_File_Ingest_InvalidException("$source is not a valid URL.");
        }
    }
}
