 ...
 * 
 * @package CollectionFiles\File\MimeType\Detect
 */
class CollectionFiles_File_MimeType_Detect
{
    /**
     ...
     * strategies must implement CollectionFiles_File_MimeType_Detect_StrategyInterface.
     */
    public function __construct($file, array $strategies = array())
    {
        // Set the file path if a File record was passed.
        if ($file instanceof CollectionFile) {
            $file = $file->getPath();
        }
        ...
        if (!is_file($file)) {
            throw new CollectionFiles_File_MimeType_Exception("The file \"$file\" does not exist.");
        }
        ...
        if (empty($strategies)) {
            ...
            $strategies = array(
                new CollectionFiles_File_MimeType_Detect_Strategy_Fileinfo,
                new CollectionFiles_File_MimeType_Detect_Strategy_FileCommand,
                new CollectionFiles_File_MimeType_Detect_Strategy_MimeContentType,
                new CollectionFiles_File_MimeType_Detect_Strategy_GetId3,
                new CollectionFiles_File_MimeType_Detect_Strategy_Browser,
            );
        } else {
            // Validate the passed strategies.
            foreach ($strategies as $strategy) {
                if (!($strategy instanceof CollectionFiles_File_MimeType_Detect_StrategyInterface)) {
                    throw new CollectionFiles_File_MimeType_Exception('Invalid MIME type detection strategy passed to Omeka_Collection_File_MimeType_Detect.');
                }
            }
        }
