<?php
class Table_BoxLocation extends Omeka_Db_Table
{
    /**
    ...
    public function findLocationByItem($item, $findOnlyOne = false)
    {
        ...
        $select = $db->select()->from(array($alias => $db->BoxLocation), "$alias.*");
        ...
        if ($findOnlyOne) {
            $boxlocation = $this->fetchObject($select);
            return $boxlocation;
        }

        // Get the locations.
        $boxlocations = $this->fetchObjects($select);
    ...
    public function getSelect()
    {
        ...
        $select->join(array('items' => $this->_db->Item), 'items.id = box_locations.item_id', array());
