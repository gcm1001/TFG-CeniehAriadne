	...
	<article class="search browse">	
		...
		<section id="results">
			...
			<?php if($searchable_types):?> 
				...
				echo '<a href="'.url('search').'" class="button sitewide-search-legend sitewide-search-edit"><span class="icon-pencil" aria-hidden="true"></span> '.__('Searching %1s of %2s Record Types',(!empty($active_types) ? count($active_types) : count($searchable_types)),count($searchable_types) ).'</a> ';
				...
				foreach($searchable_types as $record_type=>$record_label){
					$checked = (!empty($active_types) && in_array($record_type,$active_types)) ? "checked" : empty($active_types)<=0 ? "checked" : null;
